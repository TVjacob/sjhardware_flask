...
class Config:
    ...
    SQLALCHEMY_TRACK_MODIFICATIONS = False
    SQLALCHEMY_DATABASE_URI = (
        os.environ.get("DATABASE_URL")       # Docker / Render will inject
        or env.get("CLOUD_DB_URL")           # fallback for Render
        or env.get("DOCKER_DB_URL")          # fallback for Docker
        or env.get("LOCAL_DB_URL")           # fallback for local dev
        or "postgresql://postgres:password1@localhost:5432/sjhardware"
    )
